...
function calculateDuplicationScore(jscpdData) {
  if (!jscpdData || !jscpdData.statistics) {
    console.log('❌ No duplication data found or invalid format');
    return 0; // Cannot analyze = 0 score (requires manual review)
  }

  // JSCPD format: check for clones in statistics
  const stats = jscpdData.statistics;
  const totalClones = stats.clones || 0;
  const duplicatedLines = stats.duplicatedLines || 0;
  const totalLines = stats.totalLines || 1; // Avoid division by zero

  // Calculate duplication percentage
  const duplicationPercentage = (duplicatedLines / totalLines) * 100;

  // Score: 0% duplication = 100%, 15%+ duplication = 0%
  const score = Math.max(0, 100 - (duplicationPercentage * 6.67)); // 15% = 0 score

  console.log(`🔄 Duplication Score: ${score.toFixed(1)}% (${totalClones} clones, ${duplicationPercentage.toFixed(1)}% duplicated lines)`);
...
async function main() {
  ...
  const coverageData = readJsonFile(path.join(REPORTS_DIR, 'coverage', 'coverage-summary.json'));
  const eslintData = readJsonFile(path.join(REPORTS_DIR, 'eslint.changed.json'));
  const jscpdData = readJsonFile(path.join(REPORTS_DIR, 'jscpd-report.json', 'jscpd-report.json'));
