...
function Delete-WorkflowRuns {
    ...
    Write-Host "🗑️ ELIMINANDO RUNS: $status" -ForegroundColor Red
            ...
            if ($response.workflow_runs.Count -gt 0) {
                ...
                foreach ($run in $response.workflow_runs) {
                    try {
                        ...
                        Write-Host "⚠️ Error: $($run.id) - $($_.Exception.Message)" -ForegroundColor Yellow
                    }
                }
                $page++
            }
...
Write-Host "`n🔥 FASE 3: ELIMINACIÓN DE RUNS CON ERROR" -ForegroundColor Magenta
            ...
            if ($workflowRuns.workflow_runs.Count -gt 0) {
                ...
                if ($successRate -lt 50 -or $total -eq 0) {
                    ...
                    try {
                        ...
                        Write-Host "   ⚠️ Error deshabilitando: $($_.Exception.Message)" -ForegroundColor Yellow
                    }
                }
            }
        }
...
Write-Host "   🗑️ Runs fallidos eliminados: $failedDeleted" -ForegroundColor Red
Write-Host "   🗑️ Runs cancelados eliminados: $cancelledDeleted" -ForegroundColor Yellow
Write-Host "   🗑️ Runs con error eliminados: $errorDeleted" -ForegroundColor Magenta
