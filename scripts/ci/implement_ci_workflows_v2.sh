#!/usr/bin/env bash
set -euo pipefail
# ONE-STEP: crea workflows y scripts de preflight, validación cache y validator local.
# Ejecutar en la raíz del repo (VS Code). No hace push, apply ni despliegues.
BASE="$(pwd)"
PATCH_DIR="$BASE/patches_workflow_$(date --utc +%Y%m%dT%H%M%SZ)"
mkdir -p "$PATCH_DIR" "$BASE/.github/workflows" "$BASE/scripts" "$BASE/scripts/vault" || true

# 1) .github/workflows/mandatory-approval-gate.yml
...
# 2) .github/workflows/optimized-audit-parallel.yml
...
# 3) .github/workflows/integration-tests-with-compose.yml
...
# 5) scripts/validate_with_cache.sh
...
# 6) scripts/vault/validate_hmac_approval.sh (minimal local validator if absent)
...
# 7) git patch creation (no push)
git add .github/workflows/mandatory-approval-gate.yml .github/workflows/optimized-audit-parallel.yml .github/workflows/integration-tests-with-compose.yml scripts/ci_preflight.sh scripts/validate_with_cache.sh scripts/vault/validate_hmac_approval.sh || true
...
# 8) final summary (machine readable)
cat <<EOF
OK: workflows & scripts created.
Patch saved at: $PATCH_DIR
To run local preflight: ./scripts/ci_preflight.sh
If you will push, ensure audit/approval_signed.json exists and is valid:
  export VAULT_APPROVAL_KEY="your_hex_key_here"
  ./scripts/vault/validate_hmac_approval.sh audit/approval_signed.json
This script did NOT push changes.
EOF