...
let S={patched:0,env_refs_removed:0,auth_fixed:0,log_noop:0,a11y:0,count_dyn:0,login_wired:0,type_ren:0};
for(const f of files){
  ...
  s=s.replace(/<input([^>]*?)placeholder="Buscar\.\.\."([^>]*?)\/>/g,
              (m,pre,post)=>{S.a11y++;return `<input${pre}aria-label="Buscar" placeholder="Buscar..."${post}/>`;});

  // "5 agentes" → conteo real (NEURA+agentes)
  s=s.replace(/>5 agentes<\/span>/g,()=>{S.count_dyn++;return ">{dept.agents.length + 1} agentes</span>";})
     .replace(/className="text-\[11px\][^"]*">5 agentes<\/span>/g,
              ()=>{S.count_dyn++;return 'className="text-[11px] px-2 py-0.5 rounded-full bg-gray-100 border">{d.agents.length + 1} agentes</span>';});

  // Botón login cableado
  s=s.replace(/<button\s+className="h-9 px-3 rounded-lg border text-sm">INICIAR SESIÓN<\/button>/g,
              ()=>{S.login_wired++;return '<button onClick={() => window.dispatchEvent(new CustomEvent("auth:login"))}' +
              ' className="h-9 px-3 rounded-lg border text-sm">INICIAR SESIÓN</button>';});

  // Evitar choque de nombre con icono Activity
  if(/\btype\s+Activity\s*=/.test(s)){
    s=s.replace(/\btype\s+Activity\s*=/,"type ActivityEvent =")
       .replace(/useState<Activity>\(\[\]\)/g,"useState<ActivityEvent>([])")
       .replace(/useState<Activity>\[\]/g,"useState<ActivityEvent>[]");
    S.type_ren++;
  }

  if(s!==o){ fs.writeFileSync(f,s); S.patched++; }
}
// Escaneo fugas
