...
const N = s => s.replace(/\\/g, '/'), X = p => { try { fs.accessSync(p); return true } catch { return false } };
const R = f => { try { return JSON.parse(fs.readFileSync(f, 'utf8')) } catch { return null } }, T = f => { try { return fs.readFileSync(f, 'utf8') } catch { return '' } };
const sh = (c, a = [], t = 120) => spawnSync(c, a, { encoding: 'utf8', timeout: t * 1000, stdio: ['ignore', 'pipe', 'pipe'] });

const WANT = { cov: 80, dup: 4 };              // umbrales GO
const STRICT = env.STRICT_NO_DEPLOY === '1';
const MAX_FILES = +env.MAX_FILES || 120000, MAX_READ = +env.MAX_READ || 800000, MAX_DEPTH = +env.MAX_DEPTH || 7;
...
const RUN = { // auto, con override por env
  LINT: env.RUN_LINT ?? (hasBin('eslint') ? '1' : '0'),
  TESTS: env.RUN_TESTS ?? (hasBin('vitest') ? '1' : '0'),
  DUP: env.RUN_DUP ?? (X('node_modules/.bin/jscpd') ? '1' : '0'),
...
if (!workspaces.length && X('pnpm-workspace.yaml')) {
  ...
  const exp = p => /\*/.test(p) ? (b => X(b) ? fs.readdirSync(b).map(d => `${b}/${d}`) : [])(p.replace(/\/\*.*/, '')) : [p];
  workspaces = [...new Set(m.flatMap(exp))];
}

const inv = d => X(d) ? fs.readdirSync(d).filter(x => fs.statSync(path.join(d, x)).isDirectory())
  .map(x => ({ dir: N(`${d}/${x}`), name: (R(path.join(d, x, 'package.json')) || {}).name || null })) : [];
...
(function walk(root, depth = 0) {
  for (const ent of fs.readdirSync(root, { withFileTypes: true })) {
    const p = path.join(root, ent.name), n = N(p); if (IGN.test(n)) continue;
    if (ent.isDirectory()) { if (depth < MAX_DEPTH) walk(p, depth + 1); }
...
function timed(lbl, fn) { const t = Date.now(); const r = fn(); durations[lbl] = Date.now() - t; return r }

if (RUN.TESTS === '1') timed('tests', () => { const r = sh('pnpm', ['-w', '-r', 'exec', 'vitest', '--run', '--coverage', '--coverage.provider=v8'], 180);
  if (r.status === 0) { let cov = null; (function find(d) { for (const e of fs.readdirSync(d, { withFileTypes: true })) { const p = path.join(d, e.name); if (p.includes('node_modules') || p.includes('.git')) continue; if (e.isDirectory()) find(p); else if (/coverage-summary\.json$/.test(p)) cov = p } })('.');
    if (cov) { const j = R(cov); metrics.coverage_lines_pct = j?.total?.lines?.pct ?? null; } } });

if (RUN.LINT === '1') timed('eslint', () => { const r = sh('pnpm', ['-w', '-r', 'exec', 'eslint', '.', '-f', 'json'], 180);
  if (r.status === 0) { const J = JSON.parse(r.stdout || '[]'); fs.writeFileSync('reports/eslint.json', JSON.stringify(J, null, 2)); let e = 0; J.forEach(f => e += (f.errorCount || 0)); metrics.eslint_errors = e; } });

if (RUN.DUP === '1') timed('jscpd', () => { const r = sh('npx', ['--yes', 'jscpd', '--silent', '--reporters', 'json', '--output', 'reports', '.'], 180);
  if (r.status === 0) { const J = R('reports/jscpd.json'); metrics.jscpd_dup_pct = J?.statistics?.percentage ?? null; metrics.jscpd_clones_gt100 = (J?.duplicates || []).filter(d => (d?.lines || 0) > 100).length; } });

if (RUN.LEAKS === '1') timed('gitleaks', () => { const r = sh('gitleaks', ['detect', '--no-git', '-r', 'reports/gitleaks.json'], 120);
  if (r.status === 0) { const J = R('reports/gitleaks.json'); metrics.secret_leaks = Array.isArray(J?.findings) ? J.findings.length : 0; } });

if (RUN.AUDIT === '1') timed('audit', () => { const r = sh('pnpm', ['audit', '--json'], 120);
...
if (metrics.coverage_lines_pct == null && RUN.TESTS !== '1') summary.asks.push('RUN_TESTS=1 para medir cobertura');
if (metrics.eslint_errors == null && RUN.LINT !== '1') summary.asks.push('RUN_LINT=1 para medir ESLint');
if (metrics.jscpd_dup_pct == null && RUN.DUP !== '1') summary.asks.push('RUN_DUP=1 para medir duplicados');

fs.writeFileSync('reports/GO_FINAL.json', JSON.stringify(summary, null, 2));
...
console.log(JSON.stringify({ GO: summary.verdict.GO, NO_DEPLOY_OK: summary.verdict.NO_DEPLOY_OK, score, durations_ms: durations, asks: summary.asks }, null, 2));