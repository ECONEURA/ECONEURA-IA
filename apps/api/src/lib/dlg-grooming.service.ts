...
// DLQ Grooming Service - PR-72
// Sistema de gestión de errores con causas y reanudar automático
...
class DLQGroomingService {
  ...
  private patterns: Map<string, DLQPattern> = new Map();
  private retryJobs: Map<string, DLQRetryJob> = new Map();
  ...
  private startAutoGrooming() {
    // Auto-grooming runs every 5 minutes
    setInterval(() => {
  ...
  // Message Management
  async createDLQMessage(messageData: Omit<DLQMessage, 'id' | 'createdAt' | 'updatedAt'>): Promise<DLQMessage> {
    const now = new Date().toISOString();
    const newMessage: DLQMessage = {
  ...
  async getDLQMessages(organizationId: string, filters: {
    queueName?: string;
    status?: string;
    category?: string;
    severity?: string;
  ...
  // Pattern Management
  async createPattern(patternData: Omit<DLQPattern, 'id' | 'createdAt' | 'updatedAt'>): Promise<DLQPattern> {
    const now = new Date().toISOString();
    const newPattern: DLQPattern = {
  ...
  async analyzeMessage(messageId: string): Promise<DLQMessage> {
    ...
    if (matchingPatterns.length > 0) {
      ...
      message.analysis = {
        ...
        suggestedAction: bestPattern.action.type as any,
        confidence: 90,
      ...
      // Schedule action based on pattern
  ...
  private matchesPattern(message: DLQMessage, pattern: DLQPattern): boolean {
    // Check error type
  ...
  private determineRootCause(message: DLQMessage, pattern: DLQPattern): string {
  ...
  private categorizeError(message: DLQMessage, pattern: DLQPattern): 'transient' | 'permanent' | 'configuration' | 'data' | 'system' {
    const errorType = message.failureInfo.errorType;
    const errorMessage = message.failureInfo.errorMessage;
  ...
  private assessSeverity(message: DLQMessage, pattern: DLQPattern): 'low' | 'medium' | 'high' | 'critical' {
    const errorType = message.failureInfo.errorType;
  ...
  private async executeRetry(retryJob: DLQRetryJob): Promise<void> {
  ...
  private async simulateRetryProcessing(message: DLQMessage): Promise<void> {
    ...
    if (message.failureInfo.errorType === 'SMTPConnectionError') {
      ...
      // Validation errors always fail on retry
      throw new Error('Validation error persists');
    } else {
      // 50% success rate for other errors
      if (Math.random() < 0.5) {
        return; // Success
      } else {
  ...
  // Manual Grooming
  async groomMessage(messageId: string, action: {
  ...
  // Reports
  async generateReport(organizationId: string, reportType: string, startDate: string, endDate: string, generatedBy: string): Promise<DLQReport> {
