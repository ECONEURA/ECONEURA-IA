...
export class RLSPolicyGeneratorService {
  ...
  private generationRequests: PolicyGenerationRequest[] = [];
...
USING (data_sensitivity_level <= current_user_clearance_level());`,
  ...
  async generatePolicy(
    ...
    try {
      ...
      // Generate policy
      const policy = await this.generatePolicyFromTemplate(
        template,
        variables,
        rules,
        table,
  ...
  private async generatePolicyFromTemplate(
    ...
    const policy: RLSPolicy = {
      ...
      policyType: variables.policy_type as any || 'select',
      condition,
      roles,
      users,
      organizations,
  ...
  private generateCondition(rules: PolicyRule[], table: any): string {
    if (rules.length === 0) {
      return 'true';
    }
