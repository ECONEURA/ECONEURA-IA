...
// Stripe Receipts Service - PR-33
// Sistema completo de receipts de Stripe y conciliación bancaria
...
class StripeReceiptsService {
  private receipts: Map<string, StripeReceipt> = new Map();
  private reconciliationRules: Map<string, ReconciliationRule> = new Map();
  private webhookEvents: Map<string, StripeWebhookEvent> = new Map();
  ...
  private createDemoData() {
    ...
    const rule1: ReconciliationRule = {
      ...
      conditions: {
        amountRange: { min: 2000, max: 3000 }, // €20-30
        currency: 'eur',
    ...
    const rule2: ReconciliationRule = {
      ...
      conditions: {
        amountRange: { min: 10000, max: 999999 }, // €100+
        currency: 'eur'
      },
      actions: {
        autoReconcile: false,
  ...
  // Receipt Management
  async createReceipt(receiptData: Omit<StripeReceipt, 'id' | 'createdAt' | 'updatedAt' | 'reconciliationStatus'>): Promise<StripeReceipt> {
    const now = new Date().toISOString();
    const newReceipt: StripeReceipt = {
    ...
    // Auto-reconcile if rules match
    await this.autoReconcileReceipt(newReceipt.id);

    return newReceipt;
  }

  async getReceipt(receiptId: string): Promise<StripeReceipt | undefined> {
    return this.receipts.get(receiptId);
  }

  async getReceipts(organizationId: string, filters: {
    status?: string;
  ...
  // Webhook Processing
  async processWebhookEvent(event: StripeWebhookEvent): Promise<{ processed: boolean; receiptId?: string }> {
  ...
  // Reconciliation
  async autoReconcileReceipt(receiptId: string): Promise<boolean> {
  ...
  private matchesRule(receipt: StripeReceipt, rule: ReconciliationRule): boolean {
  ...
  async manualReconcileReceipt(receiptId: string, bankTransactionId: string, bankReference: string, notes?: string): Promise<StripeReceipt | undefined> {
    const receipt = this.receipts.get(receiptId);
  ...
  async createReconciliationRule(ruleData: Omit<ReconciliationRule, 'id' | 'createdAt' | 'updatedAt'>): Promise<ReconciliationRule> {
    const now = new Date().toISOString();
  ...
  // Statistics
  async getReconciliationStats(organizationId: string) {
