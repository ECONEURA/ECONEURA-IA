...
export class IntelligentRateLimiter {
  ...
  private isAllowedWithConfig(organizationId: string, config: RateLimitConfig, requestId: string): { allowed: boolean; remaining: number; resetTime: number; retryAfter?: number } {
  ...
  private fixedWindowStrategy(state: RateLimitState, config: RateLimitConfig, now: number): { allowed: boolean; remaining: number; resetTime: number } {
