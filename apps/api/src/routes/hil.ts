...
// Create a HIL task (draft) — legacy: /v1/hitl and unified: /v1/hil
...
router.patch('/v1/hitl/:id', async (req, res) => {
  ...
  if (to && !canTransition(task.state as HilState, to as HilState)) return res.status(400).json({ error: 'invalid_transition' });
  const updated = await prisma.hitl_task.update({ where: { id }, data: { state: to } });
  // record audit event (typed payload)
  try {
...
// legacy alias for /v1/hitl handled above
