...
router.get('/openapi.json', (req, res) => {
  try {
    ...
    structuredLogger.error('Failed to serve OpenAPI spec', error as Error);
    res.status(500).json({
      success: false,
...
router.get('/openapi.yaml', (req, res) => {
  try {
    ...
    structuredLogger.error('Failed to serve OpenAPI spec as YAML', error as Error);
    res.status(500).json({
      success: false,
      error: 'Failed to generate OpenAPI specification'
    });
...
router.get('/docs', (req, res) => {
  try {
    ...
    structuredLogger.error('Failed to serve API docs', error as Error);
    res.status(500).json({
      success: false,
...
router.get('/info', (req, res) => {
  try {
    ...
    structuredLogger.error('Failed to get API info', error as Error);
    res.status(500).json({
      success: false,
  ...
  for (const path in openApiSpec.paths) {
    for (const method in openApiSpec.paths[path]) {
      if (typeof openApiSpec.paths[path][method] === 'object') {
        const operation = openApiSpec.paths[path][method] as any;
        ...
        endpoints.push(`
          <div class="endpoint-item">
            ...
            <div>
              ${operation.tags ? operation.tags.map((tag: string) => `<span style="background: #f0f0f0; padding: 2px 6px; border-radius: 3px; font-size: 0.8em; margin-left: 5px;">${tag}</span>`).join('') : ''}
            </div>
          </div>
        `);
      }
