...
export class MigrationManager {
  ...
  private async applyPrismaMigrations(): Promise<void> {
    // Prisma migrations are handled by prisma migrate deploy
  ...
  private async applyRLSPolicies(): Promise<void> {
    ...
    for (const policy of policies) {
      try {
        ...
        structuredLogger.warn('RLS policy application failed', error as Error, { policy });
      }
    }
    ...
    for (const table of tables) {
      try {
        ...
        structuredLogger.warn('RLS enable failed', error as Error, { table });
      }
    }
