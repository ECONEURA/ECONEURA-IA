...
async function verifyBearer(token: string) {
  ...
  if (!jwksUri) throw new Error('JWKS_URI not configured');
...
export const requireAAD: RequestHandler = async (req, res, next) => {
  const required = (process.env.AUTH_REQUIRED ?? process.env.AAD_REQUIRED ?? 'false') === 'true';
  const header = (req.headers.authorization || '').toString();
  if (!required) return next();
  if (!header.startsWith('Bearer ')) return res.status(401).json({ code: 'unauthorized', message: 'AAD token missing' });
  ...
  try {
    ...
    const name = e?.name || '';
    if (name === 'JWTExpired' || name === 'TokenExpiredError') return res.status(401).json({ code: 'token_expired' });
