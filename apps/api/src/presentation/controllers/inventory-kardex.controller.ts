...
import {
  ...
  RecordMovementRequestSchema,
  InventoryKardexIdParamSchema,
  InventoryKardexOrganizationIdParamSchema,
  InventoryKardexSearchQuerySchema,
  InventoryKardexBulkUpdateSchema,
...
export class InventoryKardexController extends BaseController {
  private createInventoryKardexUseCase: CreateInventoryKardexUseCase;
  private updateInventoryKardexUseCase: UpdateInventoryKardexUseCase;
  private recordMovementUseCase: RecordMovementUseCase;

  constructor(private inventoryKardexRepository: InventoryKardexRepository) {
    super();
  ...
  // ========================================================================
  // INVENTORY KARDEX MANAGEMENT
  // ========================================================================

  async createInventoryKardex(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      ...
      const response: InventoryKardexResponse = this.transformInventoryKardexToResponse(result.data.inventoryKardex);
      this.sendSuccessResponse(res, response, 'Inventory kardex created successfully', 201);
    }, res, next);
  }

  async updateInventoryKardex(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
  ...
  async deleteInventoryKardex(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const { id } = InventoryKardexIdParamSchema.parse(req.params);
  ...
  async getInventoryKardex(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const { id } = InventoryKardexIdParamSchema.parse(req.params);
      ...
      const response: InventoryKardexResponse = this.transformInventoryKardexToResponse(inventoryKardex);
      this.sendSuccessResponse(res, response);
    }, res, next);
  }

  async getInventoryKardexByOrganization(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
  ...
  async searchInventoryKardex(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const { organizationId } = InventoryKardexOrganizationIdParamSchema.parse(req.params);
      const query = InventoryKardexSearchQuerySchema.parse(req.query);
  ...
  async getInventoryKardexStats(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const { organizationId } = InventoryKardexOrganizationIdParamSchema.parse(req.params);

      const stats = await this.inventoryKardexRepository.getStats(organizationId);
      const response: InventoryKardexStatsResponse = this.transformStatsToResponse(stats);
  ...
  async getInventoryByWarehouse(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const { organizationId } = InventoryKardexOrganizationIdParamSchema.parse(req.params);
  ...
  async getLowStockInventory(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const { organizationId } = InventoryKardexOrganizationIdParamSchema.parse(req.params);
      const query = InventoryKardexSearchQuerySchema.parse(req.query);
  ...
  async getOutOfStockInventory(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const { organizationId } = InventoryKardexOrganizationIdParamSchema.parse(req.params);
      const query = InventoryKardexSearchQuerySchema.parse(req.query);
  ...
  async getNeedsReorderInventory(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const { organizationId } = InventoryKardexOrganizationIdParamSchema.parse(req.params);
      const query = InventoryKardexSearchQuerySchema.parse(req.query);
  ...
  async reserveQuantity(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const { id } = InventoryKardexIdParamSchema.parse(req.params);
      ...
      inventoryKardex.reserveQuantity(
      ...
      const response: InventoryKardexResponse = this.transformInventoryKardexToResponse(updatedInventoryKardex);
  ...
  async bulkUpdateInventoryKardex(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
      const requestData = InventoryKardexBulkUpdateSchema.parse(req.body);
      const updatedBy = this.getUserId(req);

      if (requestData.updates.lowStockThreshold !== undefined) {
        await this.inventoryKardexRepository.updateThresholdsMany(
          requestData.ids,
          requestData.updates.lowStockThreshold,
          requestData.updates.reorderPoint || 0
        );
      }

      if (requestData.updates.costMethod !== undefined) {
        await this.inventoryKardexRepository.updateCostMethodMany(
          requestData.ids,
          requestData.updates.costMethod,
          requestData.updates.valuationMethod || 'fifo'
        );
      }
  ...
  async bulkDeleteInventoryKardex(req: Request, res: Response, next: NextFunction): Promise<void> {
    await this.handleAsync(async () => {
  ...
  private transformInventoryKardexToResponse(inventoryKardex: any): InventoryKardexResponse {
    return {
      ...
      movements: inventoryKardex.movements.map((movement: any) => ({
        id: movement.id,
  ...
  private transformStatsToResponse(stats: any): InventoryKardexStatsResponse {
    return {
      total: stats.total,
      active: stats.active,
