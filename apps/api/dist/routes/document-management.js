...
router.post('/', async (req, res) => {
    try {
        ...
        const document = await documentManagementService.createDocument(organizationId, validatedData, createdBy);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const document = await documentManagementService.updateDocument(id, organizationId, validatedData, updatedBy);
        if (!document) {
            return res.status(404).json({
                success: false,
                error: 'Document not found'
...
router.post('/:id/versions', async (req, res) => {
    try {
        ...
        const version = await documentManagementService.createDocumentVersion(id, organizationId, {
            ...validatedData,
            documentId: id,
            createdBy
        });
...
router.post('/:id/permissions', async (req, res) => {
    try {
        ...
        const success = await documentManagementService.grantDocumentPermission(id, organizationId, validatedData.userId, validatedData.permission, grantedBy);
        if (!success) {
            return res.status(404).json({
                success: false,
                error: 'Document not found'
...
//# sourceMappingURL=document-management.js.map