...
router.post('/optimize-query', async (req, res) => {
    try {
        ...
        const optimization = await databaseOptimizerService.optimizeQuery(validatedData.query, validatedData.params);
...
router.post('/partitions/create', async (req, res) => {
    try {
        ...
        const result = await partitionManagerService.createPartition(validatedData.tableName, validatedData.partitionName, validatedData.condition, validatedData.partitionType);
        if (result) {
