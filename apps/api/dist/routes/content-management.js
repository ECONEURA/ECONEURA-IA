...
router.post('/', async (req, res) => {
    try {
        ...
        const content = await contentManagementService.createContent(organizationId, contentData, createdBy);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const content = await contentManagementService.updateContent(id, organizationId, updates, updatedBy);
        if (!content) {
            return res.status(404).json({
                success: false,
                error: 'Content not found'
...
router.post('/:id/versions', async (req, res) => {
    try {
        ...
        const version = await contentManagementService.createContentVersion(id, organizationId, {
            ...validatedData,
            contentId: id,
            createdBy
        });
