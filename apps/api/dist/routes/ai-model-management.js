...
router.post('/models', async (req, res) => {
    try {
        ...
        structuredLogger.error('AI model creation failed', error, {
            userId: req.user?.id
        });
...
router.post('/ab-tests', async (req, res) => {
    try {
        ...
        structuredLogger.error('A/B test creation failed', error, {
            userId: req.user?.id
        });
        if (error instanceof z.ZodError) {
            return res.status(400).json({
...
router.post('/models/:id/rollback', async (req, res) => {
    try {
        ...
        const rollback = await aiModelManagementService.rollbackModel(id, validatedData.fromVersion, validatedData.toVersion, validatedData.reason, req.user?.id || 'system');
        res.status(201).json({
            success: true,
            data: rollback,
            metadata: {
...
//# sourceMappingURL=ai-model-management.js.map