...
router.get('/metrics/:name/trends', async (req, res) => {
    try {
        const { name } = req.params;
        const { period = '24h' } = req.query;
        const trends = await metricsAlertsService.getMetricTrends(name, period);
        res.json({
            success: true,
            data: trends,
            count: trends.length
...
router.get('/slas/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/slas/:id/compliance', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/reports/:period', async (req, res) => {
    try {
        ...
        const report = await metricsAlertsService.generateMetricsReport(period);
        res.json({
            success: true,
            data: report
        });
