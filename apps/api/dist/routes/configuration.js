...
const validateConfigValue = (req, res, next) => {
    try {
...
const validateFeatureFlagCheck = (req, res, next) => {
    try {
...
router.get('/feature-flags', async (req, res) => {
    try {
        ...
        const flags = await configurationService.getFeatureFlags(environment);
        res.json({
            success: true,
            data: flags,
            total: flags.length
...
router.get('/values/:key', async (req, res) => {
    try {
        ...
        let config = await configurationService.getConfigValue(key, environment);
...
router.get('/secrets/:key', async (req, res) => {
    try {
        const { key } = req.params;
        const { environment } = req.query;
        const secret = await configurationService.getSecret(key, environment);
        if (!secret) {
            return res.status(404).json({
                success: false,
...
router.delete('/secrets/:key', async (req, res) => {
    try {
        const { key } = req.params;
        const { environment } = req.query;
        const deleted = await configurationService.deleteSecret(key, environment);
        if (!deleted) {
            return res.status(404).json({
                success: false,
                error: 'Secret not found'
