...
const updateConfigSchema = z.object({
    ...
    undoWindowHours: z.number().min(1).max(168).optional(),
...
router.post('/log', async (req, res) => {
    try {
        ...
        const auditEntry = await auditTrailCRMUndoService.logAuditEntry(organizationId, userId, userEmail, validatedData.action, validatedData.resource, validatedData.resourceId, validatedData.resourceName, validatedData.before, validatedData.after, validatedData.metadata);
...
router.post('/entries/:entryId/undo', async (req, res) => {
    try {
        ...
        const userId = req.user?.id || 'system';
        const undoOperation = await auditTrailCRMUndoService.undoChange(entryId, userId, validatedData.reason, validatedData.metadata);
...
//# sourceMappingURL=audit-trail-crm-undo.js.map