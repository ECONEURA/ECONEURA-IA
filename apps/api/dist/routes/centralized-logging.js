...
import CentralizedLoggingService from '../lib/centralized-logging.service.js';
...
router.get('/logs', async (req, res) => {
    try {
        ...
        const result = await loggingService.searchLogs(validatedQuery);
        res.json({
            success: true,
            data: result.logs,
            total: result.total,
...
router.post('/logs/aggregate', async (req, res) => {
    try {
        const aggregation = {
            ...req.body,
            timeRange: {
        ...
        const result = await loggingService.aggregateLogs(validatedAggregation);
        res.json({
            success: true,
            data: result
        });
...
router.get('/retention-policies/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/retention-policies/apply', async (req, res) => {
    try {
...
router.get('/reports/:period', async (req, res) => {
    try {
        ...
        const report = await loggingService.generateLogReport(period);
        res.json({
            success: true,
            data: report
        });
...
router.get('/logs/export', async (req, res) => {
    try {
        ...
        const query = {
            ...queryParams,
            startTime: queryParams.startTime ? new Date(queryParams.startTime) : undefined,
            endTime: queryParams.endTime ? new Date(queryParams.endTime) : undefined,
            limit: queryParams.limit ? parseInt(queryParams.limit) : 10000
        };
        const validatedQuery = LogQuerySchema.parse(query);
        const result = await loggingService.searchLogs(validatedQuery);
        if (format === 'csv') {
            const csvHeaders = ['timestamp', 'level', 'message', 'service', 'environment', 'userId', 'organizationId'];
            const csvRows = result.logs.map(log => csvHeaders.map(header => `"${log[header] || ''}"`).join(','));
