...
router.get('/sse', (req, res) => {
    try {
        ...
        const userId = Array.isArray(req.headers['x-user-id']) ? req.headers['x-user-id'][0] : req.headers['x-user-id'] || 'user-demo';
        ...
        structuredLogger.error('Failed to establish cockpit SSE connection', error, {
...
router.get('/metrics/:department', (req, res) => {
    try {
        ...
        structuredLogger.error('Error getting cockpit metrics', error, {
            department: req.params.department
        });
        res.status(400).json({
            success: false,
...
router.get('/metrics/:department/history', (req, res) => {
    try {
        ...
        structuredLogger.error('Error getting cockpit metrics history', error, {
            department: req.params.department
        });
        res.status(400).json({
            success: false,
...
router.get('/agent/:agentId/status', (req, res) => {
    try {
        ...
        structuredLogger.error('Error getting agent status', error, {
            agentId: req.params.agentId
        });
        res.status(400).json({
            success: false,
...
router.get('/department/:department/agents', (req, res) => {
    try {
        ...
        structuredLogger.error('Error getting department agents', error, {
            department: req.params.department
        });
        res.status(400).json({
            success: false,
...
router.get('/events/:department/history', (req, res) => {
    try {
        ...
        structuredLogger.error('Error getting event history', error, {
            department: req.params.department
        });
        res.status(400).json({
            success: false,
...
router.get('/websocket/info', (req, res) => {
    try {
        ...
        structuredLogger.error('Error getting WebSocket info', error);
        res.status(500).json({
            success: false,
...
router.get('/status', (req, res) => {
    try {
        ...
        structuredLogger.error('Error getting cockpit status', error);
        res.status(500).json({
            success: false,
            error: 'INTERNAL_ERROR',
            message: error instanceof Error ? error.message : 'Unknown error'
...
router.get('/health', (req, res) => {
    try {
        ...
        structuredLogger.error('Error in cockpit health check', error);
        res.status(500).json({
            success: false,
...
//# sourceMappingURL=cockpit-bff-live.js.map