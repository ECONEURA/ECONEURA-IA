...
router.get('/', async (req, res) => {
    try {
        ...
        const conditions = [];
        if (filters.q) {
            conditions.push(or(ilike(contacts.firstName, `%${filters.q}%`), ilike(contacts.lastName, `%${filters.q}%`), ilike(contacts.email, `%${filters.q}%`)));
        }
...
router.post('/', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to create contact', error, {
            orgId: req.headers['x-org-id'],
...
router.put('/:id', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to update contact', error, {
            orgId: req.headers['x-org-id'],
            userId: req.headers['x-user-id'],
            contactId: req.params.id,
            body: req.body
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to delete contact', error, {
            orgId: req.headers['x-org-id'],
            userId: req.headers['x-user-id'],
            contactId: req.params.id
        });
