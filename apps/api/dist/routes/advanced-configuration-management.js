...
router.get('/configs/name/:name', async (req, res) => {
    try {
        ...
        const config = await advancedConfigurationManagementService.getConfigByName(name, environment);
        if (!config) {
            return res.status(404).json({
                success: false,
                error: 'Configuration not found'
...
router.put('/configs/:id', async (req, res) => {
    try {
        ...
        const config = await advancedConfigurationManagementService.updateConfig(id, updateData, userId || 'system');
        if (!config) {
            return res.status(404).json({
                success: false,
                error: 'Configuration not found'
...
router.delete('/configs/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { userId } = req.body;
        const success = await advancedConfigurationManagementService.deleteConfig(id, userId || 'system');
        if (!success) {
            return res.status(404).json({
                success: false,
                error: 'Configuration not found'
...
router.post('/templates/:id/generate', async (req, res) => {
    try {
        ...
        const configs = await advancedConfigurationManagementService.generateConfigFromTemplate(id, variables);
...
router.post('/deployments/:id/execute', async (req, res) => {
    try {
        const { id } = req.params;
        const { userId } = req.body;
        const result = await advancedConfigurationManagementService.executeDeployment(id, userId || 'system');
...
router.get('/configs/name/:name/value', async (req, res) => {
    try {
        const { name } = req.params;
        const { environment } = req.query;
        const value = await advancedConfigurationManagementService.getConfigValue(name, environment);
        if (value === null) {
            return res.status(404).json({
                success: false,
...
router.post('/import', async (req, res) => {
    try {
        ...
        const result = await advancedConfigurationManagementService.importConfigs(configs, environment, userId);
...
//# sourceMappingURL=advanced-configuration-management.js.map