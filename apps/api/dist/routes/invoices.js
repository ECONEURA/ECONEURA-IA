...
import { z } from 'zod';
import { CreateInvoiceSchema, UpdateInvoiceSchema, InvoiceFilterSchema } from '@econeura/shared/src/schemas/finance';
...
router.get('/', async (req, res) => {
    try {
        ...
        const conditions = [];
        if (filters.q) {
            conditions.push(or(ilike(invoices.invoiceNumber, `%${filters.q}%`), ilike(invoices.notes, `%${filters.q}%`)));
        }
        if (filters.entityId) {
        ...
        if (filters.overdue) {
            conditions.push(and(eq(invoices.status, 'overdue'), lte(invoices.dueDate, new Date().toISOString())));
        }
        ...
        const totalAmountResult = await db
            ...
            totalPaid: sql `COALESCE(SUM(paid_amount), 0)`,
            totalOutstanding: sql `COALESCE(SUM(balance_due), 0)`
        })
            .from(invoices)
        ...
        structuredLogger.error('Failed to retrieve invoices', error, {
...
router.get('/:id', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to retrieve invoice', error, {
            orgId: req.headers['x-org-id'],
...
router.post('/', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to create invoice', error, {
            orgId: req.headers['x-org-id'],
...
router.put('/:id', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to update invoice', error, {
            orgId: req.headers['x-org-id'],
            userId: req.headers['x-user-id'],
            invoiceId: req.params.id,
            body: req.body
...
router.post('/:id/send', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to send invoice', error, {
            orgId: req.headers['x-org-id'],
            userId: req.headers['x-user-id'],
            invoiceId: req.params.id
        });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to delete invoice', error, {
            orgId: req.headers['x-org-id'],
            userId: req.headers['x-user-id'],
            invoiceId: req.params.id
        });
