...
router.post('/mfa/verify-totp', async (req, res) => {
    try {
        ...
        const isValid = await mfaService.verifyTOTPCode(validatedData.userId, validatedData.code);
        if (isValid) {
...
router.post('/mfa/send-sms', async (req, res) => {
    try {
        ...
        const code = await mfaService.sendSMSCode(validatedData.userId, validatedData.phoneNumber);
...
router.post('/mfa/send-email', async (req, res) => {
    try {
        ...
        const code = await mfaService.sendEmailCode(validatedData.userId, validatedData.email);
...
router.post('/mfa/verify-code', async (req, res) => {
    try {
        ...
        const isValid = await mfaService.verifyCode(validatedData.userId, validatedData.code, validatedData.type);
        if (isValid) {
...
router.post('/mfa/verify-backup', async (req, res) => {
    try {
        ...
        const isValid = await mfaService.verifyBackupCode(validatedData.userId, validatedData.code);
        if (isValid) {
...
router.post('/mfa/session', async (req, res) => {
    try {
        ...
        const session = await mfaService.createMFASession(validatedData.userId, validatedData.requiredMethods, validatedData.ipAddress, validatedData.userAgent);
...
router.post('/mfa/complete-method', async (req, res) => {
    try {
        ...
        const isComplete = await mfaService.completeMFAMethod(validatedData.sessionId, validatedData.methodId);
...
router.post('/rbac/assign-role', async (req, res) => {
    try {
        ...
        const success = await rbacService.assignRole(validatedData.userId, validatedData.roleId, validatedData.organizationId, validatedData.assignedBy, validatedData.expiresAt ? new Date(validatedData.expiresAt) : undefined);
        if (success) {
...
router.post('/rbac/revoke-role', async (req, res) => {
    try {
        ...
        const success = await rbacService.revokeRole(validatedData.userId, validatedData.roleId, validatedData.organizationId, validatedData.revokedBy);
        if (success) {
...
router.post('/rbac/check-permission', async (req, res) => {
    try {
        ...
        const decision = await rbacService.checkPermission(validatedData.userId, validatedData.permission, validatedData.resource, validatedData.organizationId);
...
router.get('/rbac/user-roles/:userId', async (req, res) => {
    try {
        ...
        const userRoles = rbacService.getUserRoles(userId, organizationId);
        res.json({
            success: true,
            data: userRoles,
...
router.get('/rbac/user-permissions/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
        const { organizationId } = req.query;
        const permissions = rbacService.getUserPermissions(userId, organizationId);
        res.json({
            success: true,
            data: permissions,
