...
router.put('/configs/:id', async (req, res) => {
    try {
        ...
        const config = await backupRecoveryService.updateBackupConfig(id, updateData, updateData.updatedBy || 'system');
        if (!config) {
            return res.status(404).json({
                success: false,
                error: 'Backup configuration not found'
...
router.delete('/configs/:id', async (req, res) => {
    try {
        ...
        const success = await backupRecoveryService.deleteBackupConfig(id, userId || 'system');
        if (!success) {
            return res.status(404).json({
                success: false,
                error: 'Backup configuration not found'
...
router.post('/recovery', async (req, res) => {
    try {
        ...
        const recoveryJob = await backupRecoveryService.executeRecovery(backupJobId, target, options, userId);
