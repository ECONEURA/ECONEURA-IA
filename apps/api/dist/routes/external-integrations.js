...
router.post('/shipping/quote', async (req, res) => {
    ...
    try {
        ...
        const quote = await externalIntegrations.getShippingQuote(providerId, origin, destination, weight, dimensions);
        ...
        if (error instanceof z.ZodError) {
            ...
            structuredLogger.error('Shipping quote failed', error, {
                traceId,
                spanId
            });
            res.status(500).json({
...
router.get('/shipping/track/:providerId/:trackingNumber', async (req, res) => {
    ...
    try {
        ...
        const tracking = await externalIntegrations.trackShipment(validatedData.providerId, validatedData.trackingNumber);
        ...
        structuredLogger.error('Shipment tracking failed', error, {
            traceId,
            spanId
        });
        res.status(500).json({
...
router.post('/payment/process', async (req, res) => {
    ...
    try {
        ...
        const result = await externalIntegrations.processPayment(providerId, amount, currency, paymentMethod, metadata);
        ...
        if (error instanceof z.ZodError) {
            ...
            structuredLogger.error('Payment processing failed', error, {
                traceId,
                spanId
            });
            res.status(500).json({
...
router.post('/payment/refund', async (req, res) => {
    ...
    try {
        ...
        const result = await externalIntegrations.refundPayment(providerId, transactionId, amount);
        ...
        if (error instanceof z.ZodError) {
            ...
            structuredLogger.error('Payment refund failed', error, {
                traceId,
                spanId
            });
            res.status(500).json({
...
router.get('/market-data/:providerId/:symbol', async (req, res) => {
    ...
    try {
        ...
        const data = await externalIntegrations.getMarketData(validatedData.providerId, validatedData.symbol);
        ...
        structuredLogger.error('Market data request failed', error, {
            traceId,
            spanId
        });
        res.status(500).json({
...
router.post('/market-data/batch', async (req, res) => {
    ...
    try {
        ...
        const data = await externalIntegrations.getMarketDataBatch(providerId, symbols);
        ...
        if (error instanceof z.ZodError) {
            ...
            structuredLogger.error('Market data batch request failed', error, {
                traceId,
                spanId
            });
            res.status(500).json({
...
router.get('/weather/:providerId/:location', async (req, res) => {
    ...
    try {
        ...
        const data = await externalIntegrations.getWeatherData(validatedData.providerId, validatedData.location);
        ...
        structuredLogger.error('Weather data request failed', error, {
            traceId,
            spanId
        });
        res.status(500).json({
...
router.get('/weather/forecast/:providerId/:location', async (req, res) => {
    ...
    try {
        ...
        const data = await externalIntegrations.getWeatherForecast(validatedData.providerId, validatedData.location, validatedData.days);
        ...
        if (error instanceof z.ZodError) {
            ...
            structuredLogger.error('Weather forecast request failed', error, {
                traceId,
                spanId
            });
            res.status(500).json({
...
router.get('/health', async (req, res) => {
    ...
    try {
        ...
        structuredLogger.error('Integration health request failed', error, {
            traceId,
            spanId
        });
        res.status(500).json({
...
router.get('/health/:providerId', async (req, res) => {
    ...
    try {
        ...
        structuredLogger.error('Provider health request failed', error, {
            traceId,
            spanId
        });
        res.status(500).json({
