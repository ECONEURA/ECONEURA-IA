...
const PoolConfigSchema = z.object({
    ...
    idleTimeout: z.number().min(10000).max(1800000).optional(),
    connectionTimeout: z.number().min(1000).max(60000).optional(),
    acquireTimeout: z.number().min(1000).max(30000).optional(),
    ...
    circuitBreakerTimeout: z.number().min(10000).max(600000).optional(),
...
router.get('/:poolName/health', async (req, res) => {
    ...
    try {
        ...
        const healthChecks = {
            ...
            recentHealthCheck: Date.now() - poolStats.lastHealthCheck < 60000,
...
router.get('/health', async (req, res) => {
    ...
    try {
        ...
        const healthChecks = {
            ...
            recentHealthChecks: pools.every(p => Date.now() - p.lastHealthCheck < 120000)
        };
        const isHealthy = Object.values(healthChecks).every(check => check);
        const healthStatus = isHealthy ? 'healthy' : 'degraded';
