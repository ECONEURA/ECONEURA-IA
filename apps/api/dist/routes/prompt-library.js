...
router.get('/', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to list prompts', error);
        res.status(500).json({
...
router.get('/approved', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to list approved prompts', error);
        res.status(500).json({
...
router.get('/category/:category', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to get prompts by category', error, {
...
router.get('/search', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to search prompts', error);
        res.status(500).json({
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const prompt = await promptLibrary.getPrompt(id, version);
        if (!prompt) {
        ...
        structuredLogger.error('Failed to get prompt', error, {
...
router.post('/', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to add prompt', error);
        res.status(500).json({
...
router.post('/:id/approve', async (req, res) => {
    try {
        ...
        structuredLogger.error('Failed to approve prompt', error, {
            promptId: req.params.id
        });
        res.status(500).json({
