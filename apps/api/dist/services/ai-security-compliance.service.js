...
export class AISecurityComplianceService {
    ...
    async initializeDefaultPolicies() {
        ...
        for (const policy of defaultPolicies) {
            const existing = await this.db.query('SELECT id FROM ai_security_policies WHERE name = $1', [policy.name]);
            if (existing.rows.length === 0) {
                await this.db.query(`INSERT INTO ai_security_policies (name, description, type, rules, severity)
           VALUES ($1, $2, $3, $4, $5)`, [policy.name, policy.description, policy.type, JSON.stringify(policy.rules), policy.severity]);
            }
        }
    }
    async createSecurityPolicy(policy) {
        try {
            const result = await this.db.query(`INSERT INTO ai_security_policies (name, description, type, rules, severity, is_active)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`, [policy.name, policy.description, policy.type, JSON.stringify(policy.rules), policy.severity, policy.isActive]);
    ...
    async updateSecurityPolicy(id, updates) {
        try {
            const setClause = [];
            const values = [];
            let paramCount = 1;
            ...
            values.push(id);
            const result = await this.db.query(`UPDATE ai_security_policies SET ${setClause.join(', ')} WHERE id = $${paramCount} RETURNING *`, values);
    ...
    async runComplianceCheck(policyId, checkType) {
        try {
            const result = await this.db.query(`INSERT INTO ai_compliance_checks (policy_id, check_type, status)
         VALUES ($1, $2, 'running')
         RETURNING *`, [policyId, checkType]);
            const check = result.rows[0];
            const checkResult = await this.performComplianceCheck(checkType, policyId);
            await this.db.query(`UPDATE ai_compliance_checks 
         SET status = 'completed', result = $1, completed_at = NOW()
         WHERE id = $2`, [JSON.stringify(checkResult), check.id]);
    ...
    async performComplianceCheck(checkType, policyId) {
        switch (checkType) {
            case 'data_retention':
                return {
    ...
    async createSecurityIncident(incident) {
        try {
            const result = await this.db.query(`INSERT INTO ai_security_incidents (type, severity, status, description, affected_data, affected_users, detection_method, remediation)
    ...
    async getSecurityIncidents() {
        try {
            const result = await this.db.query('SELECT * FROM ai_security_incidents ORDER BY reported_at DESC');
            return result.rows.map(row => ({
                id: row.id,
                type: row.type,
                severity: row.severity,
    ...
    async logAuditEvent(auditLog) {
        try {
            const result = await this.db.query(`INSERT INTO ai_audit_logs (user_id, action, resource, resource_id, details, ip_address, user_agent, success)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         RETURNING *`, [
                auditLog.userId,
                auditLog.action,
    ...
    async evaluateAISecurity(request) {
        try {
            const violations = [];
    ...
    evaluateRule(rule, request, policy) {
        ...
        switch (rule.operator) {
            ...
            case 'regex':
                ...
                break;
            case 'range':
                if (typeof fieldValue === 'number' && fieldValue >= rule.value[0] && fieldValue <= rule.value[1])
                    return null;
                break;
            default:
                return null;
    ...
    async generateComplianceReport(organizationId, reportType, period) {
        try {
            const checks = await this.db.query(`SELECT * FROM ai_compliance_checks 
         WHERE created_at BETWEEN $1 AND $2
         ORDER BY created_at DESC`, [period.start, period.end]);
    ...
    aggregateCheckResults(checks, checkType) {
    ...
    generateRecommendations(checks) {
        ...
        const criticalViolations = checks.flatMap(check => check.result.violations.filter((v) => v.severity === 'critical'));
...
//# sourceMappingURL=ai-security-compliance.service.js.map