...
export class CustomerSupportService {
    ...
    async createTicket(organizationId, ticketData) {
        try {
    ...
    async updateTicketStatus(ticketId, organizationId, status, updatedBy) {
        try {
            const ticket = this.tickets.get(ticketId);
            if (!ticket || ticket.organizationId !== organizationId) {
                return null;
    ...
    async sendMessage(ticketId, senderId, senderType, message, messageType = 'text', attachments = []) {
        try {
    ...
    async createArticle(organizationId, articleData) {
        try {
    ...
    async searchArticles(organizationId, query, category) {
        try {
    ...
    async autoAssignTicket(ticket) {
        try {
            const availableAgents = Array.from(this.agents.values())
                .filter(agent => agent.organizationId === ticket.organizationId &&
                agent.isActive &&
    ...
    async getSupportStatistics(organizationId) {
        try {
