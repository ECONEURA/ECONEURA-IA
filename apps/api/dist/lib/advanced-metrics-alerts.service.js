...
export class AdvancedMetricsAlertsService {
    ...
    async getMetrics(filter) {
        ...
        if (filter) {
            ...
            if (filter.timeRange) {
                metrics = metrics.filter(m => m.timestamp >= filter.timeRange.start &&
    ...
    async getMetricTrends(metricName, period) {
    ...
    calculateChangePercent(metrics) {
        if (metrics.length < 2)
            return 0;
    ...
    async calculateSLACompliance(slaId, timeRange) {
    ...
    async generateMetricsReport(period) {
        const now = new Date();
    ...
    generateRecommendations(summary, alertSummary, slaSummary) {
...
//# sourceMappingURL=advanced-metrics-alerts.service.js.map