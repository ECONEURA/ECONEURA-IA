...
export class AIAgentsRegistryService {
    ...
    updateAllHealthStatuses() {
        for (const [agentId, health] of this.healthStatus) {
            ...
            if (history.length === 0)
                continue;
            const recentExecutions = history.slice(-20);
    ...
    updateHealthStatus(agentId, success, executionTime, cost) {
...
Please provide a comprehensive response that addresses the task requirements.`;
    ...
    processAgentOutputs(agent, aiResponse, inputs) {
        const baseOutput = {
            agentId: agent.id,
    ...
    estimateTokens(agent, inputs) {
        const baseTokens = 100;
        const inputTokens = JSON.stringify(inputs).length / 4;
...
//# sourceMappingURL=ai-agents-registry.service.js.map