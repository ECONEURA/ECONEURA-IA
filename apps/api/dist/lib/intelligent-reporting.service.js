export class IntelligentReportingService {
    reports = new Map();
    reportGenerations = new Map();
    scheduledReports = new Map();
    constructor() {
        this.initializeScheduledReports();
    }
    async createReport(request, organizationId, createdBy) {
        const report = {
            id: this.generateId(),
            name: request.name,
    ...
    async generateReport(reportId, generatedBy, parameters) {
        const report = this.reports.get(reportId);
        if (!report)
    ...
    async processReportGeneration(generation, report) {
        try {
    ...
    evaluateFilter(value, operator, filterValue, filterValues) {
        switch (operator) {
    ...
    async generateReportInsights(data, report) {
        const insights = [];
    ...
    async createVisualizations(data, report) {
    ...
    async createReportContent(report, data, insights, visualizations) {
        return {
            report: {
                id: report.id,
                name: report.name,
    ...
    async exportReport(content, format) {
    ...
    async updateReportSchedule(reportId, schedule) {
        await this.cancelReportSchedule(reportId);
        if (schedule.isActive) {
            await this.scheduleReport(reportId);
        }
    }
    async cancelReportSchedule(reportId) {
...
//# sourceMappingURL=intelligent-reporting.service.js.map