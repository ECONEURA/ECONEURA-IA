export class SmartCacheService {
    ...
    async updateCacheConfig(id, updates) {
    ...
    async setCacheEntry(cacheId, key, value, ttl) {
    ...
    async getCacheMetrics(cacheId) {
        return this.cacheMetrics.get(cacheId) || null;
    }
    async warmupCache(request) {
    ...
    async getCacheWarmingStatus(warmingId) {
    ...
    async executeCacheWarming(status, request) {
        try {
    ...
    async enforceCacheSizeLimit(cacheId) {
        const config = this.caches.get(cacheId);
        if (!config)
            return;
        const entries = this.cacheEntries.get(cacheId);
    ...
    async evictEntries(cacheId, targetSize) {
        const config = this.caches.get(cacheId);
        if (!config)
            return;
        const entries = this.cacheEntries.get(cacheId);
    ...
    getLRUEntries(entries, count) {
        return Array.from(entries.entries())
            .sort((a, b) => a[1].lastAccessed.getTime() - b[1].lastAccessed.getTime())
            .slice(0, count)
            .map(([key]) => key);
    }
    getLFUEntries(entries, count) {
        return Array.from(entries.entries())
            .sort((a, b) => a[1].accessCount - b[1].accessCount)
            .slice(0, count)
            .map(([key]) => key);
    }
    getTTLEntries(entries, count) {
        return Array.from(entries.entries())
            .filter(([_, entry]) => this.isEntryExpired(entry))
            .slice(0, count)
            .map(([key]) => key);
    }
    getRandomEntries(entries, count) {
    ...
    calculateTotalCacheSize(entries) {
        let totalSize = 0;
    ...
    initializeCacheMetrics(cacheId) {
        const metrics = {
            hitCount: 0,
            missCount: 0,
            hitRate: 0,
