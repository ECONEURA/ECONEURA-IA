...
econeura_cache_hits_total{cache="all"} ${Object.values(cacheStats).reduce((sum, stats) => sum + (stats?.hits || 0), 0)}

# HELP econeura_cache_misses_total Total cache misses
# TYPE econeura_cache_misses_total counter
econeura_cache_misses_total{cache="all"} ${Object.values(cacheStats).reduce((sum, stats) => sum + (stats?.misses || 0), 0)}
...
app.get("/cache/stats", (req, res) => {
    try {
        ...
        const systemStats = {
            ...
            totalHits: Object.values(stats).reduce((sum, cache) => sum + (cache?.hits || 0), 0),
            totalMisses: Object.values(stats).reduce((sum, cache) => sum + (cache?.misses || 0), 0),
            totalSize: Object.values(stats).reduce((sum, cache) => sum + (cache?.size || 0), 0),
            overallHitRate: 0
        };
