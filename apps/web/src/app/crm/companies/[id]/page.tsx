...
import { useCompany } from '@/hooks/useCRM'
...
export default function CompanyDetailPage() {
  ...
  const { data: companyData, isLoading: loadingCompany } = useCompany(companyId)
  const [page, setPage] = useState(1)
  const { data: interactionsData, isLoading: loadingInteractions, error } = useInteractions({ entityType: 'company', entityId: companyId, page, limit: 20 })
  ...
  const header = useMemo(() => (
    <div className="flex items-center justify-between">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900">{companyData?.name || 'Empresa'}</h1>
  ...
  const handleCreate = async (type: 'note' | 'email' | 'call') => {
    ...
    if (!body) return
    await createInteraction.mutateAsync({ entity_type: 'company', entity_id: companyId, type, body, subject: type === 'email' ? 'Asunto' : undefined, created_by: 'me' })
  }

  const handleSummarize = async () => {
    if (!companyId) return
    const result = await summaryMutation.mutateAsync({ entityType: 'company', entityId: companyId, limit: 10 })
  ...
  return (
    <div className="p-6">
      {loadingCompany ? (
        <div className="flex items-center gap-2"><LoadingSpinner /> Cargando empresa...</div>
      ) : (
        header
      )}
      ...
      <div className="mt-6">
        ...
        <div className="rounded-xl border border-dashed border-slate-300 p-4 min-h-[120px] max-h-[420px] overflow-y-auto" aria-live="polite" aria-busy={loadingInteractions}>
          {loadingInteractions && <div className="flex items-center gap-2"><LoadingSpinner size="sm" /> Cargando interacciones...</div>}
          {error && <p className="text-sm text-red-600">Error cargando interacciones</p>}
          {!loadingInteractions && interactions.length === 0 && <p className="text-sm text-gray-500">Sin actividad todavía.</p>}
